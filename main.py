...
async def cmd_start(message: types.Message, state: FSMContext):
    await state.finish()
    await bot.send_message(message.from_user.id, 'Привет! Я бот проекта NavMovie, помогаю с покупкой билетов в кино '
                                                 'онлайн📱. \nЧтобы пройти регистрацию, напиши /reg')
